...
class StudentStats{
	...
	public void highestGrade(Student[] studentArr, int n) {
		...
		System.out.printf("%1s\t %2s\t %3s\t",studentArr[stuIndex].id, studentArr[stuIndex].name, studentArr[stuIndex].grade);
		System.out.println();
}
	...
	public void Display(Student[] studentArr, int n) {
		for(int i = 0; i< n; i ++){
			System.out.printf("%1s\t %2s\t %3s\t",studentArr[i].id, studentArr[i].name, studentArr[i].grade);
			System.out.println();
		}
	}
}
...
public class Application{
	public static void main(String[] args) {
		...
		System.out.println("\n\n The Student with highest grade is:" );
